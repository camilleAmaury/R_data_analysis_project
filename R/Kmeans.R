kmeansfunction <- function(M, k=-1, d="euclidian", initialization="random", precision=0.001, logs=FALSE){
  ...
  #average silhouette cluster determination
  avgSilhouette <- function(clusters){
  ...
  if(k == -1){
    k_Clusters <- list()
    for(clusterNb in 2:dim(M)[1]){
      k_Clusters[[toString(clusterNb)]] <- kmeans_process(M, clusterNb, d, initialization, precision, logs)
    }
